...
# === winevulkan: WayLandIE layer injection via dispatch table replacement ===
#
# This patch modifies init_vulkan() to dlopen the WayLandIE layer .so and
# replace specific dispatch table entries (vk_funcs->p_vkXxx) with the
# layer's hooks. This is the architecturally correct way to inject a layer
# when the Android Vulkan loader cannot discover layer manifests in
# app-writable paths (non-rooted) and LD_PRELOAD is incompatible with the
# wine/FEX preloader.
#
# The layer is loaded via dlopen(RTLD_NOW) — NOT RTLD_GLOBAL — so its
# symbols do NOT interpose the global scope. Instead, we explicitly replace
# only the dispatch table entries for functions the layer intercepts:
#   vkCreateInstance, vkCreateDevice, vkCreateSwapchainKHR,
#   vkDestroySwapchainKHR, vkGetSwapchainImagesKHR,
#   vkAcquireNextImageKHR, vkQueuePresentKHR, vkCreateWin32SurfaceKHR
#
# For vkCreateInstance/vkCreateDevice, wrapper functions construct proper
# VkLayerInstanceCreateInfo/VkLayerDeviceCreateInfo chain nodes so the
# layer's PATH 1 (Khronos-compliant chain walk) is used. This passes the
# host GIPA/GDPA directly from winevulkan to the layer, avoiding the
# unreliable dlopen("libvulkan.so") in the adrenotools isolated namespace.
echo "=== Patching winevulkan: WayLandIE layer dispatch table injection ==="
python3 - <<'LAYER_INJECT_EOF'
import re
...
if "WayLandIE layer dispatch table injection" in src:
    print("  winevulkan layer injection: already applied (skipping)")
else:
    ...
    # 2. Add layer injection globals + wrapper functions after vk_funcs declaration
    old_vk_funcs = "const struct vulkan_funcs *vk_funcs;\n"
    new_vk_funcs = """const struct vulkan_funcs *vk_funcs;

/* WayLandIE layer dispatch table injection.
 *
 * Wine's vulkan.h does NOT include vk_layer.h, so we define the Khronos
 * layer chain types manually here. These must match the binary layout
 * of the same types in the layer's vk_layer.h (which the layer includes
 * via <vulkan/vk_layer.h>). */

#define WAYLANDIE_VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO ((VkStructureType)24)
#define WAYLANDIE_VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO   ((VkStructureType)25)
...
typedef struct waylandie_VkLayerDeviceLink {
    PFN_vkGetDeviceProcAddr pfnNextGetDeviceProcAddr;
} waylandie_VkLayerDeviceLink;

typedef struct waylandie_VkLayerDeviceCreateInfo {
    VkStructureType sType;
    const void *pNext;
    uint32_t function;
    union {
        waylandie_VkLayerDeviceLink *pLayerInfo;
    } u;
} waylandie_VkLayerDeviceCreateInfo;

/* g_layered_funcs is a mutable copy of vk_funcs with specific entries
 * replaced by the layer's hooks. vk_funcs is repointed to this copy
 * so all winevulkan dispatch goes through the layer for intercepted
 * functions.
 *
 * g_orig_host_gipa / g_orig_host_gdpa are saved so the wrapper
 * functions can construct layer chain nodes that give the layer access
 * to the host driver's function pointer resolution (PATH 1). */
static struct vulkan_funcs g_layered_funcs;
static void *g_waylandie_layer_handle;
static PFN_vkGetInstanceProcAddr g_orig_host_gipa;
static PFN_vkGetDeviceProcAddr g_orig_host_gdpa;
static PFN_vkCreateInstance g_layer_create_instance;
static PFN_vkCreateDevice g_layer_create_device;

/* Wrapper for vkCreateInstance: constructs a layer chain node so the
 * layer's PATH 1 (chain walk) extracts the host GIPA and calls the real
 * vkCreateInstance via it. */
static VkResult waylandie_layered_create_instance(const VkInstanceCreateInfo *ci,
        const VkAllocationCallbacks *alloc, VkInstance *inst)
{
    ...
    link.pfnNextGetInstanceProcAddr = g_orig_host_gipa;
    link.pfnNextGetDeviceProcAddr = g_orig_host_gdpa;
    ...
    return g_layer_create_instance(&patched, alloc, inst);
}

/* Wrapper for vkCreateDevice: same pattern but for the device chain. */
static VkResult waylandie_layered_create_device(VkPhysicalDevice phys,
        const VkDeviceCreateInfo *ci, const VkAllocationCallbacks *alloc, VkDevice *dev)
{
    waylandie_VkLayerDeviceLink link;
    waylandie_VkLayerDeviceCreateInfo layer_info;
    VkDeviceCreateInfo patched;

    link.pfnNextGetDeviceProcAddr = g_orig_host_gdpa;

    layer_info.sType = WAYLANDIE_VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO;
    layer_info.pNext = ci->pNext;
    layer_info.function = WAYLANDIE_VK_LAYER_LINK_INFO;
    layer_info.u.pLayerInfo = &link;
    ...
    return g_layer_create_device(phys, &patched, alloc, dev);
}
"""
    ...
    # 3. Patch init_vulkan to inject the layer at the end (before return STATUS_SUCCESS)
    old_init_end = """    *params->extensions = vk_funcs->client_extensions;
    return STATUS_SUCCESS;
}"""
    new_init_end = """    *params->extensions = vk_funcs->client_extensions;

    /* WayLandIE layer dispatch table injection.
     *
     * dlopen the layer .so (found via LD_LIBRARY_PATH which includes
     * rootDir/usr/lib/). Get the layer's GIPA via dlsym. Copy vk_funcs
     * to a mutable static and replace specific dispatch table entries
     * with the layer's hooks. For vkCreateInstance/vkCreateDevice, use
     * wrapper functions that construct proper layer chain nodes. */
    ...
    if (g_waylandie_layer_handle)
    {
        ...
        PFN_vkGetDeviceProcAddr layer_gdpa;

        g_orig_host_gipa = vk_funcs->p_vkGetInstanceProcAddr;
        g_orig_host_gdpa = vk_funcs->p_vkGetDeviceProcAddr;

        layer_gipa = (PFN_vkGetInstanceProcAddr)dlsym(g_waylandie_layer_handle, "vkGetInstanceProcAddr");
        layer_gdpa = (PFN_vkGetDeviceProcAddr)dlsym(g_waylandie_layer_handle, "vkGetDeviceProcAddr");

        if (layer_gipa)
        {
            g_layered_funcs = *vk_funcs;
            g_layer_create_instance = (PFN_vkCreateInstance)layer_gipa(NULL, "vkCreateInstance");
            g_layer_create_device = (PFN_vkCreateDevice)layer_gipa(NULL, "vkCreateDevice");

            g_layered_funcs.p_vkCreateInstance = waylandie_layered_create_instance;
            g_layered_funcs.p_vkCreateDevice = waylandie_layered_create_device;
            g_layered_funcs.p_vkCreateSwapchainKHR = (PFN_vkCreateSwapchainKHR)layer_gipa(NULL, "vkCreateSwapchainKHR");
            g_layered_funcs.p_vkDestroySwapchainKHR = (PFN_vkDestroySwapchainKHR)layer_gipa(NULL, "vkDestroySwapchainKHR");
            g_layered_funcs.p_vkGetSwapchainImagesKHR = (PFN_vkGetSwapchainImagesKHR)layer_gipa(NULL, "vkGetSwapchainImagesKHR");
            g_layered_funcs.p_vkAcquireNextImageKHR = (PFN_vkAcquireNextImageKHR)layer_gipa(NULL, "vkAcquireNextImageKHR");
            g_layered_funcs.p_vkQueuePresentKHR = (PFN_vkQueuePresentKHR)layer_gipa(NULL, "vkQueuePresentKHR");
            g_layered_funcs.p_vkCreateWin32SurfaceKHR = (PFN_vkCreateWin32SurfaceKHR)layer_gipa(NULL, "vkCreateWin32SurfaceKHR");
            if (layer_gdpa)
                g_layered_funcs.p_vkGetDeviceProcAddr = layer_gdpa;

            vk_funcs = &g_layered_funcs;
            ERR("WayLandIE layer injected into winevulkan dispatch table\\n");
        }
        else
        {
            ERR("WayLandIE layer: dlsym(vkGetInstanceProcAddr) failed: %s\\n", dlerror());
        }
    }
    else
    {
        ERR("WayLandIE layer: dlopen(libvk_layer_waylandie_dmabuf.so) failed: %s\\n", dlerror());
    }

    return STATUS_SUCCESS;
}"""
    if old_init_end not in src:
        raise SystemExit("  ERROR: could not find init_vulkan return block to patch")
    src = src.replace(old_init_end, new_init_end, 1)

    with open(path, "w") as f:
        f.write(src)
    print("  winevulkan layer injection: PATCHED")
LAYER_INJECT_EOF
