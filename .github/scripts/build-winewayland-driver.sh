...
# NOTE: We do NOT add "android" to UNEXPOSED_PLATFORMS. Adding it causes
# make_vulkan to generate vkCreateAndroidSurfaceKHR and
# vkGetAndroidHardwareBufferPropertiesANDROID thunks, which have type
# mismatches when compiled with Android NDK (AHardwareBuffer and VkSurfaceKHR
# handle types differ between Wine's definitions and Android's).
# We use VK_KHR_xlib_surface instead (already generated by default).
echo "=== Skipping android_surface patch (not needed — xlib_surface is default) ==="
