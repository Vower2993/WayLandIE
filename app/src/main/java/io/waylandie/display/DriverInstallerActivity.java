...
public final class DriverInstallerActivity extends Activity {
    ...
    private void installDriver() {
        ...
        //    into Termux inside the Debian proot.
        String inner = "waylandie-install-driver"
                + " --kind " + kind
                + " --slot " + shellQuote(slot)
                + " --file " + shellQuote("/sdcard/Download/WayLandIE/drivers/" + pickedDisplayName);
        if (chkActivate.isChecked()) {
            inner += " --activate";
        }
        String cmd = TermuxBridge.debianProotCommand(inner);
