...
public final class SetupWizardActivity extends Activity {
    ...
    static final class SetupStep {
        ...
        static final SetupStep[] STEPS = {
            ...
            new SetupStep(
                "3. Install Termux packages (bionic)",
                "Enables x11-repo + game-repo, then installs Wayland, Vulkan tools, mesa-demos, box86/64, pulseaudio, gamescope. Wine is NOT installed — Armec Proton bundles its own. ~120 MB. No root, no proot.",
                "pkg install -y x11-repo game-repo && pkg update && pkg install -y wayland wayland-protocols vulkan-tools mesa-demos box86 box64 pulseaudio gamescope wget ca-certificates"),
            new SetupStep(
                "4. Push WayLandIE scripts",
                "Scripts already extracted to /sdcard/Download/WayLandIE/linux-runtime/ (or app-private external storage). Tap to verify.",
                "ls -la ~/storage/downloads/WayLandIE/linux-runtime/install.sh 2>/dev/null || ls -la /sdcard/Android/data/io.waylandie.display/files/Download/WayLandIE/linux-runtime/install.sh"),
            new SetupStep(
                "5. Install WayLandIE helpers",
                "Runs install.sh --backend termux-native. Installs waylandie-* scripts into $PREFIX/bin and sets up PulseAudio audio bridge.",
                "cd ~/storage/downloads/WayLandIE/linux-runtime 2>/dev/null || cd /sdcard/Android/data/io.waylandie.display/files/Download/WayLandIE/linux-runtime; sh install.sh --backend termux-native --prefix $PREFIX --install-packages"),
            new SetupStep(
                ...
                null),
            new SetupStep(
                "7. Run waylandie-doctor",
                "Verifies bridge socket, Turnip driver (if installed), audio bridge, and a test render of vkcube.",
                "waylandie-doctor"),
        };
    }
