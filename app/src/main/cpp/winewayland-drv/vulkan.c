                                              ...
                                              VkSurfaceKHR *handle, struct client_surface **client)
{
...
#ifdef __ANDROID__
    /* On Android, the adrenotools Vulkan wrapper supports VK_KHR_xlib_surface.
     * It translates xlib_surface to android_surface internally.
     * We use vkCreateXlibSurfaceKHR with the ANativeWindow as the "window" param.
     * The wrapper ignores the Display* and uses the ANativeWindow directly.
     */
    {
    ...
    if (g_anative_window)
    {
        ...
        create_info.window = (Window)(uintptr_t)g_anative_window;  /* ANativeWindow* as Window */
