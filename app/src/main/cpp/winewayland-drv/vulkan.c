...
/* vkCreateAndroidSurfaceKHR is now generated by make_vulkan (after adding
 * "android" to UNEXPOSED_PLATFORMS). The instance struct has
 * p_vkCreateAndroidSurfaceKHR directly — no need for dlopen/dlsym. */
                                              ...
                                              VkSurfaceKHR *handle, struct client_surface **client)
{
    ...
    if (g_anative_window)
    {
        ...
        res = instance->p_vkCreateAndroidSurfaceKHR(instance->host.instance, &create_info, NULL, handle);
        if (res != VK_SUCCESS)
        {
...
#ifdef __ANDROID__
    /* Map win32_surface → android_surface so DXVK's request for
     * VK_KHR_win32_surface is satisfied by VK_KHR_android_surface. */
